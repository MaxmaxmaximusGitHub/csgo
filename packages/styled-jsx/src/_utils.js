...
export const makeStyledJsxTag = (id, transformedCss, expressions = [], path) => {
...
function _addImportUrls(css, {file, expressions}) {

  css = css.replace(urlRegExp, (match, quote, url) => {

    if (url.indexOf('%%styled-jsx-placeholder-') === 0) {
      return match
    }

    const exp = parse(`require(${ JSON.stringify(url) }).default || require(${ JSON.stringify(url) })`).program.body[0].expression
    const expIndex = expressions.push(exp) - 1

    return `url(%%styled-jsx-placeholder-${ expIndex }%%)`
  })
...
function addImportUrls(css, stylesInfo) {
  if (typeof css === 'string') {
    return _addImportUrls(css, stylesInfo)
  }

  return css.map(s => _addImportUrls(s, stylesInfo))
}
...
export const processCss = (stylesInfo, options) => {
  ...
  transformedCss = addImportUrls(transformedCss, stylesInfo)

  if (Array.isArray(transformedCss)) {
    if (expressions.length > 0) {
    ...
    transformedCss = templateLiteralFromPreprocessedCss(transformedCss,
      expressions
    )
  }
